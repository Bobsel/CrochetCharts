...
| S5 | **Undo via Qt's `QUndoStack`** with ≈ 18 typed `QUndoCommand` subclasses in `src/crochetchartcommands.cpp`. | Uniform undo/redo across every mutating operation. Every new mutator requires a matching command class. |
